...
check()
{
	...
	LinearAlgebra::distributed::Vector<double> solution_domain, solution_interface, solution_C;
	assembly_helper.get_dof_handler_system().split_vector(solution, solution_domain, solution_interface, solution_C);
	...
	Vector<double> solution_domain_seq, solution_interface_seq, solution_C_seq;
	assembly_helper_seq.get_dof_handler_system().split_vector(solution_seq, solution_domain_seq, solution_interface_seq, solution_C_seq);

	//compare norms because comparing dof values is difficult due to different numbering for sequential and parallel dof handling
	const double norm = solution_domain.l2_norm() + solution_interface.l2_norm() + solution_C.l2_norm();
	const double norm_seq = solution_domain_seq.l2_norm() + solution_interface_seq.l2_norm() + solution_C_seq.l2_norm();
	if(tria_system.get_this_proc_n_procs().first == 0)
		deallog << "Norms of parallel and sequential versions: " << norm << " / " << norm_seq << endl << endl;
