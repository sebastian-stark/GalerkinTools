...
check()
{
	...
	Vector<double> solution_domain_0, solution_interface_0, solution_C_0;
	assembly_helper.get_dof_handler_system().split_vector(solution, solution_domain_0, solution_interface_0, solution_C_0);

	const unsigned int n_dofs_domain = assembly_helper.get_dof_handler_system().n_dofs_domain();
	const unsigned int n_dofs_interface = assembly_helper.get_dof_handler_system().n_dofs_interface();
	const unsigned int n_dofs_additional = assembly_helper.get_dof_handler_system().n_dofs_additional();
	dof_renumbering.add_range(0, n_dofs_domain - 1, n_dofs_interface);
	dof_renumbering.add_range(n_dofs_domain, n_dofs_domain + n_dofs_interface - 1, -n_dofs_domain);
	dof_renumbering.add_range(n_dofs_domain + n_dofs_interface, n_dofs_domain + n_dofs_interface + n_dofs_additional - 1, 0);
	assembly_helper.get_dof_handler_system().attach_dof_renumbering(dof_renumbering);
	solution = 0.0;

	assembly_helper.get_initial_fields_vector(solution);
	Vector<double> solution_domain_1, solution_interface_1, solution_C_1;
	assembly_helper.get_dof_handler_system().split_vector(solution, solution_domain_1, solution_interface_1, solution_C_1);

	double error = 0;

	auto index_1 = solution_domain_1.begin();
	for(const auto& index_0 : solution_domain_0)
	{
		error += fabs(index_0 - *index_1);
		++index_1;
	}

	index_1 = solution_interface_1.begin();
	for(const auto& index_0 : solution_interface_0)
	{
		error += fabs(index_0 - *index_1);
		++index_1;
	}

	index_1 = solution_C_1.begin();
	for(const auto& index_0 : solution_C_0)
	{
		error += fabs(index_0 - *index_1);
		++index_1;
	}
