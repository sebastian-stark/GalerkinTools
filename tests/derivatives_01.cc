...
check()
{
	...
	for(const auto& cell_domain : tria_domain.cell_iterators_on_level(0))
		for(unsigned int face = 0; face < dealii::GeometryInfo<spacedim>::faces_per_cell; ++face)
			if(cell_domain->face(face)->at_boundary())
				tria_system.add_interface_cell(cell_domain, face, 0);

	SphericalManifold<spacedim-1, spacedim> spherical_manifold;
	tria_system.set_interface_manifold(0, spherical_manifold);
	tria_system.close();
