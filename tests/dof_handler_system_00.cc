...
check()
{
	...
	//setup sequential triangulation
	dealii::Triangulation<spacedim> tria_domain_seq(dealii::Triangulation<spacedim>::limit_level_difference_at_vertices);
	GridGenerator::subdivided_hyper_cube(tria_domain_seq,2,-1.0,1.0);
	dealii::GalerkinTools::TriangulationSystem<spacedim> tria_system_seq(tria_domain_seq);
	for(const auto& domain_cell : tria_domain_seq.cell_iterators_on_level(0))
	{
		for(unsigned int face = 0; face<dealii::GeometryInfo<spacedim>::faces_per_cell; ++face)
		{
			if( (domain_cell->center()[0] < 0.) && (domain_cell->face(face)->center()[0] > -1e-14 ) )
				...
				tria_system_seq.add_interface_cell(domain_cell, face, 0);
			else if( (domain_cell->center()[0] > 0.) && (domain_cell->face(face)->center()[0] > 1.-1e-14 ) )
				tria_system_seq.add_interface_cell(domain_cell, face, 2);
			else if( (domain_cell->center()[1] < 0.) && (domain_cell->face(face)->center()[1] > -1e-14 ) )
				tria_system_seq.add_interface_cell(domain_cell, face, 4);
			else if( (domain_cell->center()[1] < 0.) && (domain_cell->face(face)->center()[1] < -1.+1e-14 ) )
				tria_system_seq.add_interface_cell(domain_cell, face, 3);
			else if( (domain_cell->center()[1] > 0.) && (domain_cell->face(face)->center()[1] > 1.-1e-14 ) )
				tria_system_seq.add_interface_cell(domain_cell, face, 5);
		}
	}
	...
	for(unsigned int cycle = 0; cycle < cycles; ++cycle)
	{
		...
		for(const auto& domain_cell : tria_domain_seq.active_cell_iterators())
			if( (domain_cell->center()[0] < 0.) && (domain_cell->center()[1] < 0.))
				if( spacedim == 2 )
					domain_cell->set_refine_flag();
				else if( domain_cell->center()[2] < 0. )
	...
	dealii::GalerkinTools::DoFHandlerSystem<spacedim> dof_handler_system_seq(tria_system_seq);
	...
	for(const auto& domain_cell : dof_handler_system_seq.get_dof_handler_domain().active_cell_iterators())
		if(domain_cell->is_locally_owned())
		{
			if(domain_cell->center()[0] < 0.)
				domain_cell->set_active_fe_index(0);
	...
	for(const auto& interface_cell : dof_handler_system_seq.get_dof_handler_interface().active_cell_iterators())
		if(interface_cell->is_locally_owned())
		{
			if(interface_cell->center()[0] < -1e-12)
				interface_cell->set_active_fe_index(0);
	...
	dof_handler_system_seq.distribute_dofs(fe_collection_domain, fe_collection_interface);

	//compare
	deallog << dof_handler_system.get_dof_handler_interface().n_dofs() << " " << dof_handler_system_seq.get_dof_handler_interface().n_dofs() << endl;
	deallog << dof_handler_system.get_dof_handler_domain().n_dofs() << " " << dof_handler_system_seq.get_dof_handler_domain().n_dofs() << endl;
}
